"""Patch Ask FIXED workflow inside the n8n Docker volume (path /data)."""
import json
import sqlite3
...
EXPORT = Path("/export/NewsDash_Ask_Agent_DM.json")
...
# Activate FIXED; deactivate other ask variants to avoid double replies.
cur.execute(
...
db.commit()
names = [n["name"] for n in json.loads(cur.execute("SELECT nodes FROM workflow_entity WHERE id=?", (WF,)).fetchone()[0])]
active = cur.execute("SELECT active FROM workflow_entity WHERE id=?", (WF,)).fetchone()[0]
print("patched", names, "active=", active)
db.close()
